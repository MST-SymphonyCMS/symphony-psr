...
$Page->addStylesheetToHead(APP_URL . '/assets/css/symphony.forms.css', 'screen', 32);
...
$actions->appendChild(
    Widget::Input('action[delete]', tr('Uninstall extension'), 'submit', array(
        'accesskey' => 'd',
        'class' => 'button delete',
        'style' => 'margin-left: 0;',
        'title' => tr('Uninstall this extension'),
    )
));
...
if ($match != "" && $e->getAdditional()->rename_failed !== true) {
    ...
    $button = new XMLElement('button', tr('Rename folder'));
    ...
    $div->appendChild(
        new XMLElement('p', tr('You can try uninstalling the extension to continue, or you might want to ask on the forums'))
    );
}
