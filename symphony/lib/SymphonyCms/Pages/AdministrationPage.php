...
class AdministrationPage extends HTMLPage
{
    ...
    public function build(array $context = array())
    {
        ...
        $this->addStylesheetToHead(APP_URL . '/assets/css/symphony.legacy.css', 'screen', 31);
        $this->addStylesheetToHead(APP_URL . '/assets/css/symphony.grids.css', 'screen', 32);
        $this->addStylesheetToHead(APP_URL . '/assets/css/symphony.forms.css', 'screen', 33);
        $this->addStylesheetToHead(APP_URL . '/assets/css/symphony.tables.css', 'screen', 34);
        $this->addStylesheetToHead(APP_URL . '/assets/css/symphony.frames.css', 'screen', 35);
        $this->addStylesheetToHead(APP_URL . '/assets/css/symphony.tabs.css', 'screen', 36);
        $this->addStylesheetToHead(APP_URL . '/assets/css/symphony.drawers.css', 'screen', 37);
        $this->addStylesheetToHead(APP_URL . '/assets/css/symphony.associations.css', 'screen', 38);
        $this->addStylesheetToHead(APP_URL . '/assets/css/symphony.notices.css', 'screen', 39);
        $this->addStylesheetToHead(APP_URL . '/assets/css/admin.css', 'screen', 40);

        $this->addScriptToHead(APP_URL . '/assets/js/jquery.js', 50);
        $this->addScriptToHead(APP_URL . '/assets/js/symphony.js', 60);
        $this->addScriptToHead(APP_URL . '/assets/js/symphony.collapsible.js', 61);
        $this->addScriptToHead(APP_URL . '/assets/js/symphony.orderable.js', 62);
        $this->addScriptToHead(APP_URL . '/assets/js/symphony.selectable.js', 63);
        $this->addScriptToHead(APP_URL . '/assets/js/symphony.duplicator.js', 64);
        $this->addScriptToHead(APP_URL . '/assets/js/symphony.tags.js', 65);
        $this->addScriptToHead(APP_URL . '/assets/js/symphony.suggestions.js', 66);
        $this->addScriptToHead(APP_URL . '/assets/js/symphony.pickable.js', 67);
        $this->addScriptToHead(APP_URL . '/assets/js/symphony.timeago.js', 68);
        $this->addScriptToHead(APP_URL . '/assets/js/symphony.notify.js', 69);
        $this->addScriptToHead(APP_URL . '/assets/js/symphony.drawer.js', 70);
        $this->addScriptToHead(APP_URL . '/assets/js/admin.js', 80);
    ...
    public function canAccessPage()
    {
        ...
        foreach ($nav as $item) {
            if (// If page directly matches one of the children
                General::inArrayMulti($page, $item['children'])
                // If the page namespace matches one of the children (this will usually drop query
                // string parameters such as /edit/1/)
                or General::inArrayMulti(Symphony::getPageNamespace() . '/', $item['children'])
            ) {
    ...
    private function appendBodyId()
    {
        ...
        $body_id = strtolower(get_class($this));
     ...
     * `switchboard()` which acts a bit of a controller to show content based on
     ...
     * `switchboard` with the type set to 'action'.
     *
     * @see switchboard()
     */
    ...
    /**
     * The `switchboard` function acts as a controller to display content
     ...
     * `actionNew()`. If an action function is not provided by the Page, this function
            ...
            if (count($children) > 0) {
                foreach ($children as $child) {
                    $item = array(
                        ...
                        'name' => tr(strval($child->attributes()->name)),
    ...
    private function buildSectionNavigation(&$nav)
    {
        ...
        if (is_array($sections) && !empty($sections)) {
            foreach ($sections as $s) {
                $group_index = self::navigationFindGroupIndex($nav, $s->get('navigation_group'));

                if ($group_index === false) {
                    $group_index = General::arrayFindAvailableIndex($nav, 0);
                ...
                foreach ($extension_navigation as $item) {
                    ...
                    switch ($type) {
                        case Extension::NAV_GROUP:
                            $index = General::arrayFindAvailableIndex($nav, $item['location']);
                        ...
                        case Extension::NAV_CHILD:
                            ...
                            if (!is_numeric($item['location'])) {
                                ...
                                $group_index = self::navigationFindGroupIndex($nav, $item['location']);
                            } else {
    ...
    public function buildNavigation()
    {
        ...
        if (!$found) {
            self::findActiveNavigationGroup($nav, '/^' . str_replace('/', '\/', $pageCallback['pageroot']) . '/i', true);
        }
     ...
     */
    private static function navigationFindGroupIndex(array $nav, $group)
    {
     ...
     */
    private static function findActiveNavigationGroup(array &$nav, $pageroot, $pattern = false)
    {
    ...
    public function appendUserLinks()
    {
        ...
        $li = new XMLElement('li');
        $li->appendChild(Widget::Anchor(tr('Log out'), SYMPHONY_URL . '/logout/', null, null, null, array('accesskey' => 'l')));
        $ul->appendChild($li);
     ...
     */
    public function fetchAvailablePageTypes()
    {
