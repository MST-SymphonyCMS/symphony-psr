...
abstract class ResourcesPage extends AdministrationPage
{
    ...
    public function viewIndex($resource_type)
    {
        ...
        if (!is_array($resources) || empty($resources)) {
            ...
            foreach ($resources as $r) {
                ...
                if (isset($r['source'], $r['source']['id'])) {
                    ...
                    $class = call_user_func(array($manager, 'getClassName'), $r['handle']);
                    ...
                    $section = Widget::TableData(tr('Unknown'), 'inactive');
                }
                ...
                if ($pages == '') {
                    $pagelinks = Widget::TableData(tr('None'), 'inactive');
                } else {
        ...
        $group_attach = array('label' => tr('Attach to Page'), 'options' => array());
        $group_detach = array('label' => tr('Detach from Page'), 'options' => array());

        $group_attach['options'][] = array('attach-all-pages', false, tr('All'));
        $group_detach['options'][] = array('detach-all-pages', false, tr('All'));
            ...
            if (is_array($checked) && !empty($checked)) {
                if ($_POST['with-selected'] == 'delete') {
                    ...
                    foreach ($checked as $handle) {
                        $path = call_user_func(array($manager, 'getDriverPath'), $handle);
                        ...
                        if (!General::deleteFile($path)) {
                            ...
                            $this->pageAlert(
                                tr('Failed to delete %s.', array('<code>' . basename($path) . '</code>'))
                                . ' ' . tr('Please check permissions on %s', array('<code>' . $folder . '</code>')),
                                Alert::ERROR
                            );
                            $canProceed = false;
                        } else {
