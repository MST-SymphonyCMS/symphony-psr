...
use \SymphonyCms\Pages\XSLTPage;
use \SymphonyCms\Toolkit\DatasourceManager;
use \SymphonyCms\Toolkit\EventManager;
...
class FrontendPage extends XSLTPage
{
     ...
     * by `\SymphonyCms\Utilities\getCurrentPage`
     *
     * @var string
     ...
     * @see buildPage()
     * @param string $page
     * The URL of the current page that is being Rendered as returned by \SymphonyCms\Utilities\getCurrentPage
     * @return string
    ...
    public function generate($page = null)
    {
        ...
        if ($full_generate) {
            ...
            if (is_null($devkit)) {
                if (General::inIarray('XML', $this->_pageData['type'])) {
                    $this->addHeaderToPage('Content-Type', 'text/xml; charset=utf-8');
                } elseif (General::inIarray('JSON', $this->_pageData['type'])) {
    ...
    private function buildPage()
    {
        ...
        if (is_array($_GET) && !empty($_GET)) {
            foreach ($_GET as $key => $val) {
                ...
                if (is_array($val)) {
                    $val = General::arrayMapRecursive(array('FrontendPage', 'sanitizeParameter'), $val);
                } else {
        ...
        foreach ($this->_param as $key => $value) {
            ...
            } elseif ($key == 'current-query-string') {
    ...
    public function resolvePage($page = null)
    {
        ...
        if ((!$this->_page || $this->_page == '//') && is_null($row)) {
            ...
            if (empty($pathArr)) {
                ...
                if (!$this->isSchemaValid($index['params'], $page_extra_bits)) {
                    return false;
                } else {
                    $row = $index;
                }
            } elseif (!$this->isSchemaValid($row['params'], $page_extra_bits)) {
        ...
        if (!$this->is_logged_in && in_array('admin', $row['type'])) {
            ...
            if (empty($row)) {
                Frontend::instance()->throwCustomError(
                    tr('Please login to view this page.') . ' <a href="' . SYMPHONY_URL . '/login/">' . tr('Take me to the login page') . '</a>.',
                    tr('Forbidden'),
     ...
     *  of the $bits array.
     */
    private function isSchemaValid($schema, array $bits)
    {
     ...
     * page in the correct order determined by `findEventOrder()`. The results
     ...
     * @return integer
     */
    private function findEventOrder($a, $b)
    {
     ...
     */
    private function findDatasourceOrder($dependenciesList)
    {
        ...
        do {
            ...
            foreach ($dependenciesList as $handle => $dependencies) {
                if (General::inArrayAll(array_map(create_function('$a', "return str_replace('\$ds-', '', \$a);"), $dependencies), $orderedList)) {
                    $orderedList[] = str_replace('_', '-', $handle);
                    unset($dependenciesList[$handle]);
                }
            }
