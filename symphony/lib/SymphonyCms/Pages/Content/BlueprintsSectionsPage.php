 ...
 */
class BlueprintsSectionsPage extends AdministrationPage
{
    ...
    public function viewIndex()
    {
        ...
        $aTableHead = array(
            array(tr('Name'), 'col'),
            array(tr('Entries'), 'col'),
            array(tr('Navigation Group'), 'col')
        );
        ...
        $options = array(
            ...
            array('delete', false, tr('Delete'), 'confirm', null, array(
                'data-message' => tr('Are you sure you want to delete the selected sections?')
            )),
            array('delete-entries', false, tr('Delete Entries'), 'confirm', null, array(
                'data-message' => tr('Are you sure you want to delete all entries in the selected sections?')
            ))
        );

        if (is_array($sections) && !empty($sections)) {
            $index = 3;
            $options[$index] = array('label' => tr('Set navigation group'), 'options' => array());
    ...
    public function viewNew()
    {
        ...
        $this->insertBreadcrumbs(
            array(
                Widget::Anchor(tr('Sections'), SYMPHONY_URL . '/blueprints/sections/'),
            )
        );
        ...
        if (!isset($meta['navigation_group'])) {
            $meta['navigation_group'] = (isset($this->_navigation[0]['name']) ? $this->_navigation[0]['name'] : tr('Content'));
        }
        ...
        $label->setValue(tr('%s Hide this section from the back-end menu', array($input->generate(false))));
        $namediv->appendChild($label);
        $div->appendChild($namediv);

        $navgroupdiv = new XMLElement('div', null, array('class' => 'column'));
        $sections = SectionManager::fetch(null, 'ASC', 'sortorder');
        $label = Widget::Label(tr('Navigation Group'));
        ...
        $fieldset = new XMLElement('fieldset');
        $fieldset->setAttribute('class', 'settings');
        $fieldset->appendChild(new XMLElement('legend', tr('Fields')));

        $div = new XMLElement('div', null, array('class' => 'frame'));

        $ol = new XMLElement('ol');
        $ol->setAttribute('id', 'fields-duplicator');
        $ol->setAttribute('data-add', tr('Add field'));
        $ol->setAttribute('data-remove', tr('Remove field'));
        ...
        $div->appendChild(Widget::Input('action[save]', tr('Create Section'), 'submit', array('accesskey' => 's')));
    ...
    public function viewEdit()
    {
        ...
        if ($formHasErrors) {
            ...
            switch($this->_context[2]) {
                ...
                case 'created':
                    $this->pageAlert(
                        tr('Section created at %s.', array(DateTimeObj::getTimeAgo()))
                        . ' <a href="' . SYMPHONY_URL . '/blueprints/sections/new/" accesskey="c">'
                        . tr('Create another?')
                        . '</a> <a href="' . SYMPHONY_URL . '/blueprints/sections/" accesskey="a">'
                        . tr('View all Sections')
        ...
        $this->setPageType('form');
        $this->setTitle(tr('%1$s &ndash; %2$s &ndash; %3$s', array($meta['name'], tr('Sections'), tr('Symphony'))));
        $this->appendSubheading(
            $meta['name'],
            Widget::Anchor(tr('View Entries'), SYMPHONY_URL . '/publish/' . $section->get('handle'), tr('View Section Entries'), 'button')
        );
        $this->insertBreadcrumbs(
            array(
                Widget::Anchor(tr('Sections'), SYMPHONY_URL . '/blueprints/sections/'),
        ...
        $button->setAttributeArray(array('name' => 'action[delete]', 'class' => 'button confirm delete', 'title' => tr('Delete this section'), 'type' => 'submit', 'accesskey' => 'd', 'data-message' => tr('Are you sure you want to delete this section?')));
    ...
    public function actionNew()
    {
        if (@array_key_exists('save', $_POST['action']) || @array_key_exists('done', $_POST['action'])) {
            ...
            if (!isset($meta['name']) || strlen(trim($meta['name'])) == 0) {
                ...
                if ($meta['name'] !== $existing_section->get('name')
                    && !is_null($s) && $s !== $section_id
                ) {
                    $this->_errors['name'] = tr('A Section with the name %s already exists', array('<code>' . $meta['name'] . '</code>'));
                    $canProceed = false;
                }
            ...
            if (!isset($meta['navigation_group']) || strlen(trim($meta['navigation_group'])) == 0) {
                $this->_errors['navigation_group'] = tr('This is a required field.');
                $canProceed = false;
            }
                ...
                if ($canProceed) {
                    ...
                    foreach ($fields as $position => $data) {
                        ...
                        if (trim($data['element_name']) != '' && in_array($data['element_name'], $name_list)) {
                            $this->_errors[$position] = array('element_name' => tr('A field with this handle already exists. All handle must be unique.'));
                            $canProceed = false;
                            break;
                        }
                ...
                if ($canProceed) {
                    ...
                    foreach ($fields as $position => $data) {
                        ...
                        if ($field->mustBeUnique() && !in_array($field->get('type'), $unique)) {
                            ...
                            $canProceed = false;
                            $this->_errors[$position] = array('label' => tr('There is already a field of type %s. There can only be one per section.', array('<code>' . $field->handle() . '</code>')));
                        }
