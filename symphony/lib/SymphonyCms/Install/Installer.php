...
// Hacks for function_exists
use \mysqlconnect;

/**
 * Symphony CMS Installer
 *
 * @package SymphonyCms
 * @subpackage Install
 */
class Installer extends Administration
{
    ...
    public function __construct()
    {
        ...
        $settings = include __DIR__.'/includes/config_default.php';
        ...
        // Initialize logs
        $this->initialiseLog(LOGS . '/install');
    ...
    {
        if (!(self::$instance instanceof Installer)) {
    ...
    public function run()
    {
        ...
        if (is_null(Symphony::Log()) || !file_exists(Symphony::Log()->getLogPath())) {
            self::render(new InstallerPage('missing-log'));
        }
        ...
        if (!isset($_POST['lang']) && count(Lang::getAvailableLanguages(false)) > 1) {
            self::render(new InstallerPage('languages'));
        }
        ...
        if (isset($_POST['fields'])) {
            ...
            if (!empty($errors)) {
                ...
                $disabled_extensions = self::install();
    ...
    private static function checkRequirements()
    {
        ...
        if (version_compare(phpversion(), '5.2', '<=')) {
            $errors[] = array(
                'msg' => tr('PHP Version is not correct'),
                'details' => tr('Symphony requires %1$s or greater to work, however version %2$s was detected.', array('<code><abbr title="PHP: Hypertext Pre-processor">PHP</abbr> 5.2</code>', '<code>' . phpversion() . '</code>'))
            );
        }

        // Make sure the install.sql file exists
        if (!file_exists(__DIR__.'/includes/install.sql') || !is_readable(__DIR__.'/includes/install.sql')) {
            $errors[] = array(
                'msg' => tr('Missing install.sql file'),
                'details'  => tr('It appears that %s is either missing or not readable. This is required to populate the database and must be uploaded before installation can commence. Ensure that PHP has read permissions.', array('<code>install.sql</code>'))
            );
        }
        ...
        if (!function_exists('mysql_connect')) {
            $errors[] = array(
                'msg' => tr('MySQL extension not present'),
                'details'  => tr('Symphony requires MySQL to work.')
            );
        }
        ...
        if (!extension_loaded('zlib')) {
            $errors[] = array(
                'msg' => tr('ZLib extension not present'),
                'details' => tr('Symphony uses the ZLib compression library for log rotation.')
            );
        }
        ...
        if (!extension_loaded('xml') && !extension_loaded('libxml')) {
            $errors[] = array(
                'msg' => tr('XML extension not present'),
                'details'  => tr('Symphony needs the XML extension to pass data to the site frontend.')
            );
        }
        ...
        if (!extension_loaded('xsl') && !extension_loaded('xslt') && !function_exists('domxml_xslt_stylesheet')) {
            $errors[] = array(
                'msg' => tr('XSLT extension not present'),
                'details'  => tr('Symphony needs an XSLT processor such as %s or Sablotron to build pages.', array('Lib<abbr title="eXtensible Stylesheet Language Transformation">XSLT</abbr>'))
            );
        }
        ...
        if (!function_exists('json_decode')) {
            $errors[] = array(
                'msg' => tr('JSON functionality is not present'),
                'details'  => tr('Symphony uses JSON functionality throughout the backend for translations and the interface.')
            );
        }
        ...
        if (!is_writable(DOCROOT)) {
            $errors['no-write-permission-root'] = array(
                ...
                'details' => tr('Symphony does not have write permission to the root directory. Please modify permission settings on %s. This can be reverted once installation is complete.', array('<code>' . DOCROOT . '</code>'))
            );
        }
        ...
        if (is_dir(DOCROOT . '/workspace') && !is_writable(DOCROOT . '/workspace')) {
            $errors['no-write-permission-workspace'] = array(
                ...
                'details' => tr('Symphony does not have write permission to the existing %1$s directory. Please modify permission settings on this directory and its contents to allow this, such as with a recursive %2$s command.', array('<code>/workspace</code>', '<code>chmod -R</code>'))
            );
        }
    ...
    private static function checkConfiguration()
    {
        ...
        try {
            ...
            if ($e->getDatabaseErrorCode() === 1044 or $e->getDatabaseErrorCode() === 1045) {
                $errors['database-invalid-credentials'] = array(
                    ...
                    'details' => tr('Symphony was unable to access the database with these credentials.')
                );
            } else {
                $errors['no-database-connection'] = array(
                    'msg' => 'Could not establish database connection.',
                    'details' => tr('Symphony was unable to establish a valid database connection. You may need to modify host or port settings.')
                );
            }
        }
        ...
        try {
            if (Symphony::Database()->isConnected()) {
                ...
                if (version_compare($version, '5.0', '<')) {
                    $errors['database-incorrect-version']  = array(
                        ...
                        'details' => tr('Symphony requires %1$s or greater to work, however version %2$s was detected. This requirement must be met before installation can proceed.', array('<code>MySQL 5.0</code>', '<code>' . $version . '</code>'))
                    );
                } else {
                    ...
                    if (is_array($tables) && !empty($tables)) {
                        $errors['database-table-clash']  = array(
                            ...
                            'details' =>  tr('The table prefix %s is already in use. Please choose a different prefix to use with Symphony.', array('<code>' . $fields['database']['tbl_prefix'] . '</code>'))
                        );
                    }
                }
            }
            ...
            $errors['unknown-database']  = array(
                ...
                'details' =>  tr('Symphony was unable to connect to the specified database.')
            );
        }
        ...
        if (trim($fields['general']['sitename']) == '') {
            $errors['general-no-sitename']  = array(
                ...
                'details' => tr('You must enter a Site name. This will be shown at the top of your backend.')
            );
        }
        ...
        if (trim($fields['user']['username']) == '') {
            $errors['user-no-username']  = array(
                ...
                'details' => tr('You must enter a Username. This will be your Symphony login information.')
            );
        }

        // Password Not Entered else mismatched
        if (trim($fields['user']['password']) == '') {
            $errors['user-no-password']  = array(
                'msg' => 'No password entered.',
                'details' => tr('You must enter a Password. This will be your Symphony login information.')
            );
        } elseif ($fields['user']['password'] != $fields['user']['confirm-password']) {
            $errors['user-password-mismatch']  = array(
                'msg' => 'Passwords did not match.',
                'details' => tr('The password and confirmation did not match. Please retype your password.')
            );
        }
        ...
        if (trim($fields['user']['firstname']) == '' || trim($fields['user']['lastname']) == '') {
            $errors['user-no-name']  = array(
                ...
                'details' =>  tr('You must enter your name.')
            );
        }
        ...
        if (!preg_match('/^\w(?:\.?[\w%+-]+)*@\w(?:[\w-]*\.)+?[a-z]{2,}$/i', $fields['user']['email'])) {
            $errors['user-invalid-email']  = array(
                ...
                'details' =>  tr('This is not a valid email address. You must provide an email address since you will need it if you forget your password.')
            );
        }
    ...
    protected static function abort($message, $start)
    {
        ...
        self::render(new InstallerPage('failure'));
    }
    ...
    private static function install()
    {
        ...
        $htaccess = str_replace(
            '<!-- REWRITE_BASE -->',
            $rewrite_base,
            file_get_contents(__DIR__.'/includes/htaccess.txt')
        );
    ...
    protected static function render(InstallerPage $page)
    {
