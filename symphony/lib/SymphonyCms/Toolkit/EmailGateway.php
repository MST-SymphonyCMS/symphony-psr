...
use \SymphonyCms\Exceptions\EmailValidationException;
use \SymphonyCms\Toolkit\EmailHelper;
...
abstract class EmailGateway
{
    ...
    {
        if (preg_match('%[\r\n]%', $email)) {
            throw new EmailValidationException(tr('Sender Email Address can not contain carriage return or newlines.'));
        }
    ...
    {
        if (preg_match('%[\r\n]%', $name)) {
            throw new EmailValidationException(tr('Sender Name can not contain carriage return or newlines.'));
        }
    ...
    public function setTextEncoding($encoding = null)
    {
        if ($encoding == 'quoted-printable') {
            ...
            throw new EmailGatewayException(tr('%1$s is not a supported encoding type. Please use %2$s or %3$s. You can also use %4$s for no encoding.', array($encoding, '<code>quoted-printable</code>', '<code>base-64</code>', '<code>false</code>')));
        }
    }
    ...
    {
        if (preg_match('%[\r\n]%', $email)) {
            throw new EmailValidationException(tr('Reply-To Email Address can not contain carriage return or newlines.'));
        }
    ...
    {
        if (preg_match('%[\r\n]%', $name)) {
            throw new EmailValidationException(tr('Reply-To Name can not contain carriage return or newlines.'));
        }
    ...
    {
        if (is_array($body)) {
            throw new EmailGatewayException(tr('%s accepts strings only; arrays are not allowed.', array('<code>appendHeaderField</code>')));
        }
    ...
    public function validate()
    {
        if (strlen(trim($this->_subject)) <= 0) {
            throw new EmailValidationException(tr('Email subject cannot be empty.'));
        } elseif (strlen(trim($this->sender_email_address)) <= 0) {
            throw new EmailValidationException(tr('Sender email address cannot be empty.'));
        } else {
            foreach ($this->_recipients as $address) {
                if (strlen(trim($address)) <= 0) {
                    throw new EmailValidationException(tr('Recipient email address cannot be empty.'));
                } elseif (!filter_var($address, FILTER_VALIDATE_EMAIL)) {
                    throw new EmailValidationException(tr('The email address ‘%s’ is invalid.', array($address)));
                }
            }
        }
    ...
    protected function prepareMessageBody()
    {
        if (!empty($this->_attachments)) {
            ...
            throw new EmailGatewayException(tr('No attachments or body text was set. Can not send empty email.'));
        }
    }
    ...
    {
        if (method_exists(get_class($this), 'set'.$this->toCamel($name, true))) {
            return $this->{'set'.$this->toCamel($name, true)}($value);
        } else {
            throw new EmailGatewayException(tr('The %1$s gateway does not support the use of %2$s', array(get_class($this), $name)));
        }
    }
     ...
     * @return string
     */
    private function toCamel($string, $caseFirst = false)
    {
    ...
    /**
     * The reverse of the toCamel function.
     *
     * @param string $string
     *  The string to convert
     * @return string
     */
    private function fromCamel($string)
    {
