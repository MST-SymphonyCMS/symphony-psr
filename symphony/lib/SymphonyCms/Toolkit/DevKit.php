...
class DevKit extends HTMLPage
{
    ...
    protected function buildIncludes()
    {
        ...
        $this->addStylesheetToHead(APP_URL . '/assets/css/devkit.css', 'screen');
    }
    ...
    protected function buildHeader(XMLElement $wrapper)
    {
        ...
        $h1->appendChild(
            Widget::Anchor(
                $this->_pagedata['title'],
                ($this->_query_string ? '?' . trim(html_entity_decode($this->_query_string), '&') : '.')
            )
        );
        ...
        if ($root->hasChildNodes()) {
            foreach ($root->childNodes as $item) {
                if ($item->tagName == 'item') {
                    $item->setAttribute('name', tr($item->getAttribute('name')));
                }
            }
        }
