...
class MySQL
{
    ...
    {
        MySQL::$connection['tbl_prefix'] = $prefix;
    }
    ...
    {
        return (isset(MySQL::$connection['id']) && is_resource(MySQL::$connection['id']));
    }
    ...
    {
        if ($this->isConnected()) {
            return mysql_close(MySQL::$connection['id']);
        }
    }
    ...
    public function query($query, $type = "OBJECT")
    {
        ...
        if (mysql_error()) {
            $this->error();
        } elseif (is_resource($this->_result)) {
            if ($type == "ASSOC") {
        ...
        $stop = precisionTimer('stop', $start);
    ...
    private function error()
    {
        ...
        throw new DatabaseException(
            tr('MySQL Error (%1$s): %2$s in query: %3$s', array($errornum, $msg, $this->_lastQuery)),
            array(
                'msg' => $msg,
                'num' => $errornum,
                'query' => $this->_lastQuery
