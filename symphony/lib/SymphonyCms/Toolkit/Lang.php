...
class Lang
{
    ...
    public static function initialize()
    {
        ...
        if (empty(self::$_datetime_dictionary)) {
            self::$_datetime_dictionary = include __DIR__.'/includes/datetime.php';
        }

        // Load default transliterations
        if (empty(self::$_transliterations)) {
            self::$_transliterations = include __DIR__.'/includes/transliterations.php';
        }
        ...
        if ($code != 'en' && (self::isLanguageEnabled($code) || $checkStatus == false)) {
            ...
            if ($code != 'en' && class_exists('Symphony') && Symphony::Log() instanceof Log) {
                Symphony::Log()->pushToLog(
                    tr('The selected language, %s, could not be found. Using default English dictionary instead.', array($code)),
                    E_ERROR,
                    true
                );
            }
    ...
    /**
     * This function is an internal alias for `tr()`.
     *
     * @since Symphony 2.3
     * @see toolkit.tr()
     * @param string $string
 ...
 * @var integer
 */
defineSafe('LANGUAGE_ENABLED', 10);
 ...
 * @var integer
 */
defineSafe('LANGUAGE_DISABLED', 11);
