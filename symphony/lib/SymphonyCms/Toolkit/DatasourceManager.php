 ...
 */
class DatasourceManager implements FileResourceInterface
{
    /**
    ...
    {
        return strtolower(preg_replace('/.php$/i', '', $filename));
    }
    ...
    {
        return ucwords($handle) . 'Datasource';
    }
        ...
        if (is_file(DATASOURCES . "/$handle.php")) {
            ...
            if (is_array($extensions) && !empty($extensions)) {
                foreach ($extensions as $e) {
                    if (is_file(EXTENSIONS . "/$e/data-sources/$handle.php")) {
     ...
     *
     * @see DatasourceManager::getClassPath()
     * @param string $handle
     *  The handle of the Datasource free from any Symphony conventions
     *  such as `data.*.php`
     * @return string
    ...
    {
        $result = array();
        $structure = General::listStructure(DATASOURCES, '/[\\w-]+.php/', false, 'ASC', DATASOURCES);
    ...
    public static function create($handle, array $env = null, $process_params = true)
    {
        ...
        if (!is_file($path)) {
            throw new Exception(
                tr('Could not find Data Source %s.', array('<code>' . $handle . '</code>'))
