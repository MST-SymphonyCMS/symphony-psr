 ...
 */
class FieldManager implements FileResourceInterface
{
    /**
     ...
     * @return string
     */
    public static function getClassPath($type)
    {
        if (is_file(TOOLKIT . "/fields/field.{$type}.php")) {
            ...
            if (is_array($extensions) && !empty($extensions)) {
                foreach ($extensions as $e) {
                    if (is_file(EXTENSIONS . "/{$e}/fields/{$type}.php")) {
     ...
     * @return string
     */
    public static function getDriverPath($type)
    {
        return self::getClassPath($type) . "/{$type}.php";
    }
    ...
    public static function fetch($id = null, $section_id = null, $order = 'ASC', $sortfield = 'sortorder', $type = null, $location = null, $where = null, $restrict = Field::__FIELD_ALL__)
    {
        ...
        if (!empty($field_ids) || is_null($id)) {
            ...
            foreach ($result as $f) {
                ...
                if (isset(self::$_initialiased_fields[$f['id']])
    ...
    public static function listAll()
    {
        ...
        if (is_array($extensions) && !empty($extensions)) {
            ...
            $structure['filelist'] = General::arrayRemoveDuplicates($structure['filelist']);
        }

        $types = array();

        foreach ($structure['filelist'] as $filename) {
            $types[] = self::getHandleFromFilename($filename);
        }
    ...
    public static function create($type)
    {
        if (!isset(self::$_pool[$type])) {
            ...
            $path = self::getDriverPath($type);

            if (!file_exists($path)) {
                throw new Exception(
                    tr('Could not find Field %1$s at %2$s.', array('<code>' . $type . '</code>', '<code>' . $path . '</code>'))
