...
use \SymphonyCms\Toolkit\TextFormatter;
 ...
 */
class TextformatterManager implements FileResourceInterface
{
    /**
    ...
    {
        return self::getClassPath($handle) . "/formatter.$handle.php";
    }
    ...
    public static function create($handle)
    {
        if (!isset(self::$_pool[$handle])) {
            ...
            if (!is_file($path)) {
                throw new Exception(
                    tr('Could not find Text Formatter %s.', array('<code>' . $classname . '</code>'))
