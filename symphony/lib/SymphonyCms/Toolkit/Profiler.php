...
class Profiler
{
    ...
    {
        Profiler::$_seed = (is_null($time) ? precisionTimer() : $time);
    }
    ...
    public function sample($msg, $type = PROFILE_RUNNING_TOTAL, $group = 'General', $queries = null)
    {
        if ($type == PROFILE_RUNNING_TOTAL) {
            Profiler::$_samples[] = array($msg, precisionTimer('stop', Profiler::$_starttime), precisionTimer(), $group, $queries, memory_get_usage());
        } else {
            ...
            Profiler::$_samples[] = array($msg, precisionTimer('stop', ($start ? $start : $prev[2])), precisionTimer(), $group, $queries, memory_get_usage());
        }
    }
 ...
 * @var integer
 */
defineSafe('PROFILE_RUNNING_TOTAL', 0);
