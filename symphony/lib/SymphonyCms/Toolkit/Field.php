...
class Field
{
    ...
    public function displaySettingsPanel(XMLElement &$wrapper, $errors = null)
    {
        ...
        $label = (($this->get('label')) ? $this->get('label') : tr('New Field'));
    ...
    public function buildLocationSelect($selected = null, $name = 'fields[location]', $label_value = null)
    {
        ...
        $options = array(
            array('main', $selected == 'main', tr('Main content')),
            array('sidebar', $selected == 'sidebar', tr('Sidebar'))
        );
    ...
    public function buildFormatterSelect($selected = null, $name = 'fields[format]', $label_value)
    {
        ...
        $options[] = array('none', false, tr('None'));
    ...
    public function buildValidationSelect(XMLElement &$wrapper, $selected = null, $name = 'fields[validator]', $type = 'input')
    {
        ...
        $rules = ($type == 'upload' ? Validators::upload() : Validators::string());

        $label = Widget::Label(tr('Validation Rule'));
        $label->setAttribute('class', 'column');
    ...
    public function appendRequiredCheckbox(XMLElement &$wrapper)
    {
        ...
        $label->setValue(tr('%s Make this a required field', array($input->generate())));
    ...
    public function appendShowColumnCheckbox(XMLElement &$wrapper)
    {
        ...
        $label->setValue(tr('%s Display in entries table', array($input->generate())));
    ...
    public function checkFields(array &$errors, $checkForDuplicates = true)
    {
        ...
        if ($this->get('label') == '') {
            $errors['label'] = tr('This is a required field.');
        }

        if ($this->get('element_name') == '') {
            $errors['element_name'] = tr('This is a required field.');
        } elseif (!$valid_name) {
            $errors['element_name'] = tr('Invalid element name. Must be valid %s.', array('<code>QName</code>'));
        } elseif ($checkForDuplicates) {
            if (FieldManager::fetchFieldIDFromElementName($element_name, $parent_section) != $this->get('id')) {
                $errors['element_name'] = tr('A field with that element name already exists. Please choose another.');
            }
        }
    ...
    public function displayDatasourceFilterPanel(XMLElement &$wrapper, $data = null, $errors = null, $fieldnamePrefix = null, $fieldnamePostfix = null)
    {
        $wrapper->appendChild(
            new XMLElement(
                'header',
                '<h4>' . $this->get('label') . '</h4> <span>' . $this->name() . '</span>',
                array(
     ...
     *
     * @see toolkit.Datasource#determineFilterType
     * @param array $data
    ...
    {
        throw new Exception(
            tr('Data source output grouping is not supported by the %s field', array('<code>' . $this->get('label') . '</code>'))
        );
    }
