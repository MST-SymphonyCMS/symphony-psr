...
use \SymphonyCms\Exceptions\EmailException;
...
abstract class SectionEvent extends Event
{
    ...
    public static function appendErrors(XMLElement $result, array $fields, $errors, $post_values)
    {
        ...
        $result->appendChild(new XMLElement('message', tr('Entry encountered errors when saving.')));

        foreach ($errors as $field_id => $message) {
            $field = FieldManager::fetch($field_id);

            if (is_array($fields[$field->get('element_name')])) {
                $type = array_reduce($fields[$field->get('element_name')], array('SectionEvent', 'reduceType'));
            } else {
     ...
     *  'missing' or 'invalid'
     */
    public function reduceType($a, $b)
    {
     ...
     * Multiple, before invoking the `doIt` function, which actually
     * processes the Event. Once the Event has executed, this function will
     * determine if the user should be SymphonyCmsTILITIESREDIRECTed to a URL, or to just return
     * the XML.
     *
     * @return XMLElement|void
     *  If `$_REQUEST{'SymphonyCmsTILITIESREDIRECT']` is set, and the Event executed successfully,
     *  the user will be SymphonyCmsTILITIESREDIRECTed to the given location. If `$_REQUEST['SymphonyCmsTILITIESREDIRECT']`
    ...
    public function execute()
    {
        ...
        if (in_array('expect-multiple', $this->eParamFILTERS)) {
            ...
            $success = $this->doIt($fields, $result, null, $entry_id);
        }
     ...
     */
    public function doIt($fields, XMLElement &$result, $position = null, $entry_id = null)
    {
        ...
        if (!$section = SectionManager::fetch($this->getSource())) {
            $result->setAttribute('result', 'error');
            $result->appendChild(new XMLElement('message', tr('The Section, %s, could not be found.', array($this->getSource()))));
        ...
        if (is_array($fields) && !empty($fields)) {
            General::arrayToXml($post_values, $fields, true);
        }
        ...
        if (isset($entry_id)) {
            ...
            if (!is_object($entry)) {
                $result->setAttribute('result', 'error');
                $result->appendChild(new XMLElement('message', tr('The Entry, %s, could not be found.', array($entry_id))));
        ...
        if (__ENTRY_FIELD_ERROR__ == $entry->checkPostData($fields, $errors, ($entry->get('id') ? true : false))) {
            ...
            $result->setAttribute('result', 'error');
            $result->appendChild(new XMLElement('message', tr('Unknown errors where encountered when saving.')));
            ...
            $result->appendChild(
                new XMLElement(
                    'message',
                    (isset($entry_id) ? tr('Entry edited successfully.') : tr('Entry created successfully.'))
                )
            );
        }
    ...
    public function processSendMailFilter(XMLElement $result, array $send_email, array &$fields, Section $section, Entry $entry)
    {
        ...
        foreach ($fields['recipient'] as $recipient) {
            ...
            if (empty($author)) {
                $errors['recipient'][$recipient] = tr('Recipient not found');
                continue;
            }
