 ...
 */
class FieldUpload extends Field implements ExportableFieldInterface, ImportableFieldInterface
{
    ...
    public static function getMetaInfo($file, $type)
    {
        ...
        if (General::inIarray($type, FieldUpload::$imageMimeTypes) && $array = @getimagesize($file)) {
            $meta['width'] = $array[0];
            $meta['height'] = $array[1];
        }
    ...
    public function displaySettingsPanel(XMLElement &$wrapper, $errors = null)
    {
        ...
        $label = Widget::Label(tr('Destination Directory'));
    ...
    public function checkFields(array &$errors, $checkForDuplicates = true)
    {
        if (is_dir(DOCROOT . $this->get('destination') . '/') === false) {
            ...
            $errors['destination'] = tr('The destination directory is not writable.')
                . ' '
                . tr('Please check permissions on %s.', array(
                    '<code>' . $this->get('destination') . '</code>'
                ));
        }
    ...
    public function displayPublishPanel(XMLElement &$wrapper, $data = null, $flagWithError = null, $fieldnamePrefix = null, $fieldnamePostfix = null, $entry_id = null)
    {
        if (is_dir(DOCROOT . $this->get('destination') . '/') === false) {
            ...
            $flagWithError = tr('Destination folder is not writable.') . ' ' . tr(
        ...
        if (isset($data['file'])) {
            ...
            if (file_exists($file) === false || !is_readable($file)) {
                $flagWithError = tr('The file uploaded is no longer available. Please check that it exists, and is readable.');
            }
    ...
    public function checkPostFieldData($data, &$message, $entry_id = null)
    {
        ...
        if (is_array($data) === false) {
            ...
            if (file_exists($file) === false || !is_readable($file)) {
                $message = tr('The file uploaded is no longer available. Please check that it exists, and is readable.');
            ...
            if ($this->get('validator') != null) {
                ...
                if (General::validateString($file, $rule) === false) {
                    $message = tr('File chosen in ‘%s’ does not match allowable file types for that field.', array(
                        $this->get('label')
                    ));
        ...
        if (is_dir(DOCROOT . $this->get('destination') . '/') === false) {
            ...
            $message = tr('Destination folder is not writable.') . ' ' . tr(
                'Please check permissions on %s.',
                array(
                    '<code>' . $this->get('destination') . '</code>'
                )
        ...
        if ($data['error'] != UPLOAD_ERR_NO_FILE && $data['error'] != UPLOAD_ERR_OK) {
            switch ($data['error']) {
                case UPLOAD_ERR_INI_SIZE:
                    $message = tr('File chosen in ‘%1$s’ exceeds the maximum allowed upload size of %2$s specified by your host.', array($this->get('label'), (is_numeric(ini_get('upload_max_filesize')) ? General::formatFilesize(ini_get('upload_max_filesize')) : ini_get('upload_max_filesize'))));
                    break;
                case UPLOAD_ERR_FORM_SIZE:
                    $message = tr('File chosen in ‘%1$s’ exceeds the maximum allowed upload size of %2$s, specified by Symphony.', array($this->get('label'), General::formatFilesize($_POST['MAX_FILE_SIZE'])));
                    break;
                case UPLOAD_ERR_PARTIAL:
                case UPLOAD_ERR_NO_TMP_DIR:
                    $message = tr('File chosen in ‘%s’ was only partially uploaded due to an error.', array($this->get('label')));
                    break;
                case UPLOAD_ERR_CANT_WRITE:
                    $message = tr('Uploading ‘%s’ failed. Could not write temporary file to disk.', array($this->get('label')));
                    break;
                case UPLOAD_ERR_EXTENSION:
                    $message = tr('Uploading ‘%s’ failed. File upload stopped by extension.', array($this->get('label')));
                    break;
            }
        ...
        if ($this->get('validator') != null) {
            ...
            if (!General::validateString($data['name'], $rule)) {
                $message = tr('File chosen in ‘%s’ does not match allowable file types for that field.', array($this->get('label')));
    ...
    public function appendFormattedElement(XMLElement &$wrapper, $data, $encode = false, $mode = null, $entry_id = null)
    {
        ...
        $item->setAttributeArray(
            array(
                'size' => (
                    file_exists($file)
                    && (is_readable($file) ? General::formatFilesize(filesize($file)) : 'unknown')
                ),
    ...
    {
        return array(
            'getFilename' =>    ExportableFieldInterface::VALUE,
            'getObject' =>      ExportableFieldInterface::OBJECT,
