 ...
 */
class FieldSelect extends Field implements ExportableFieldInterface, ImportableFieldInterface
{
    public function __construct()
    {
        parent::__construct();

        $this->_name = tr('Select Box');
        $this->_required = true;
    ...
    public function displaySettingsPanel(XMLElement &$wrapper, $errors = null)
    {
        ...
        // Predefined Values
        $label = Widget::Label(tr('Predefined Values'));
        ...
        // Dynamic Values
        $label = Widget::Label(tr('Dynamic Values'));
        $label->setAttribute('class', 'column');
        $label->appendChild(new XMLElement('i', tr('Optional')));
        ...
        foreach ($field_groups as $group) {
            ...
            foreach ($group['fields'] as $f) {
                if ($f->get('id') != $this->get('id') && $f->canPrePopulate()) {
                    $fields[] = array($f->get('id'), ($this->get('dynamic_options') == $f->get('id')), $f->get('label'));
                }
            }
        ...
        $label->setValue(tr('%s Allow selection of multiple options', array($input->generate())));
        $div->appendChild($label);

        $this->appendShowAssociationCheckbox($div, tr('available when using Dynamic Values'));
        ...
        $label->setValue(tr('%s Sort all options alphabetically', array($input->generate())));
        $div->appendChild($label);
        $wrapper->appendChild($div);
    ...
    public function checkFields(array &$errors, $checkForDuplicates = true)
    {
        ...
        if ($this->get('static_options') == '' && ($this->get('dynamic_options') == '' || $this->get('dynamic_options') == 'none')) {
            $errors['dynamic_options'] = tr('At least one source must be specified, dynamic or static.');
        }
    ...
    {
        if (!is_array($data) or is_null($data['value'])) {
            return;
        }
