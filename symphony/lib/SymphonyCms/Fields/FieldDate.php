 ...
 * input into it.
 */
class FieldDate extends Field implements ExportableFieldInterface, ImportableFieldInterface
{
    const SIMPLE = 0;
    const REGEXP = 1;
    const RANGE = 3;
    ...
    public function displaySettingsPanel(XMLElement &$wrapper, $errors = null)
    {
        ...
        $label->setValue(tr('%s Pre-populate with current date', array($input->generate())));
    ...
    public function checkPostFieldData($data, &$message, $entry_id = null)
    {
        ...
        if (!DateTimeObj::validate($data)) {
            $message = tr('The date specified in ‘%s’ is invalid.', array($this->get('label')));
