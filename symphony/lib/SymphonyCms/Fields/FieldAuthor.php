 ...
 */
class FieldAuthor extends Field implements ExportableFieldInterface
{
    public function __construct()
    {
        parent::__construct();

        $this->_name = tr('Author');
        $this->_required = true;
     ...
     */
    private static function parseFilter($value)
    {
    ...
    public function displaySettingsPanel(XMLElement &$wrapper, $errors = null)
    {
        ...
        $label = Widget::Label(tr('Author types'));
        $label->setAttribute('class', 'column');
        $types = $this->get('author_types');
        $options = array(
            array('author', empty($types) ? true : in_array('author', $types), tr('Author')),
            array('manager', empty($types) ? true : in_array('manager', $types), tr('Manager')),
            array('developer', empty($types) ? true : in_array('developer', $types), tr('Developer'))
        );
        $label->appendChild(
            Widget::Select(
                'fields['.$this->get('sortorder').'][author_types][]',
                $options,
                array(
                    'multiple' => 'multiple'
                )
        ...
        $label->setValue(tr('%s Allow selection of multiple authors', array($input->generate())));
        $div->appendChild($label);
        ...
        $label->setValue(tr('%s Select current user by default', array($input->generate())));
        $div->appendChild($label);
        $wrapper->appendChild($div);
    ...
    public function checkFields(array &$errors, $checkForDuplicates = true)
    {
        ...
        if (empty($types)) {
            $errors['author_types'] = tr('This is a required field.');
        }
    ...
    {
        $value = $this->prepareExportValue($data, ExportableFieldInterface::LIST_OF + ExportableFieldInterface::VALUE, $entry_id);
    ...
    {
        return $this->prepareExportValue($data, ExportableFieldInterface::LIST_OF + ExportableFieldInterface::AUTHOR, $entry_id);
    }
    ...
    {
        return array(
            'listAuthor' =>  ExportableFieldInterface::LIST_OF + ExportableFieldInterface::AUTHOR,
            'listAuthorObject' => ExportableFieldInterface::LIST_OF + ExportableFieldInterface::AUTHOR + ExportableFieldInterface::OBJECT,
            'listAuthorToValue' => ExportableFieldInterface::LIST_OF + ExportableFieldInterface::AUTHOR + ExportableFieldInterface::VALUE,
            'listValue' => ExportableFieldInterface::LIST_OF + ExportableFieldInterface::VALUE,
        ...
        if (self::isFilterRegex($data[0])) {
            ...
            $where .= "
                AND (
                    ...
                    OR t{$field_id}_{$this->_key}authors.username {$regex} '{$pattern}'
                )
            ";
            ...
            foreach ($data as $value) {
                ...
                if (FieldAuthor::parseFilter($value) == "author_id") {
                    $where .= "
                    ...
                    $where .= "
                        AND t{$field_id}_{$this->_key}authors.username = '{$value}'
                    ";
                }
            }
            ...
            $where .= "
                AND (
                    ...
                    OR
                    t{$field_id}_{$this->_key}authors.username IN ('{$data}')
                    )
            ";
        }
