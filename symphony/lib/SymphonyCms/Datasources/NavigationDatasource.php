...
{
    public function processNavigationParentFilter($parent)
    {
    ...
    public function processNavigationTypeFilter($filter, $filter_type = DataSource::FILTER_OR)
    {
    ...
    public function execute(array &$param_pool = null)
    {
        ...
        if (trim($this->dsParamFILTERS['type']) != '') {
            $type_sql = $this->processNavigationTypeFilter($this->dsParamFILTERS['type'], $this->determineFilterType($this->dsParamFILTERS['type']));
        }

        if (trim($this->dsParamFILTERS['parent']) != '') {
            $parent_sql = $this->processNavigationParentFilter($this->dsParamFILTERS['parent']);
        }
