...
class DynamicXmlDatasource extends Datasource
{
    public function execute(array &$param_pool = null)
    {
        ...
        if (isset($this->dsParamXPATH)) {
            $this->dsParamXPATH = $this->processParametersInString($this->dsParamXPATH, $this->_env);
        }
        ...
        if ((!is_array($cachedData) || empty($cachedData)) // There's no cache.
            ...
            if (Mutex::acquire($cache_id, $timeout, TMP)) {
                ...
                if ((int)$info['http_code'] != 200 || !preg_match('/(xml|plain|text)/i', $info['content_type'])) {
                    ...
                    if ($writeToCache === false) {
                        ...
                        $result->appendChild(new XMLElement('error', tr('Data returned is invalid.')));
                ...
                $result->appendChild(
                    new XMLElement('error', tr('The %s class failed to acquire a lock, check that %s exists and is writable.', array(
                        '<code>Mutex</code>',
                        '<code>' . TMP . '</code>'
                    )))
                );
        ...
        if (!$this->_force_empty_result && is_object($result)) {
            ...
            if ($proc->isErrors()) {
                $result->setAttribute('valid', 'false');
                $error = new XMLElement('error', tr('Transformed XML is invalid.'));
