...
use \SymphonyCms\Exception\FrontendPageNotFoundException;
...
use \SymphonyCms\Fields\FieldDate;
...
class SectionDatasource extends Datasource
{
    ...
    public function processEntry(Entry $entry)
    {
        ...
        if ($this->_canprocess_system_parameters) {
    ...
    public function processFilters(&$where, &$joins, &$group)
    {
        ...
        foreach ($this->dsParamFILTERS as $field_id => $filter) {
            ...
            if (!is_array($filter)) {
                $filter_type = $this->determineFilterType($filter);
    ...
    public function execute(array &$param_pool = null)
    {
        ...
        if (!$section = SectionManager::fetch((int)$this->getSource())) {
            $about = $this->about();
            trigger_error(tr('The Section, %s, associated with the Data source, %s, could not be found.', array($this->getSource(), '<code>' . $about['name'] . '</code>')), E_USER_ERROR);
        }

        $sectioninfo = new XMLElement(
            'section',
        ...
        $this->_canprocess_system_parameters = $this->canProcessSystemParameters();
