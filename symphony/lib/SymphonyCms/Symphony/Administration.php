            ...
            if (is_array($extensions) && !empty($extensions) && $this->canAccessAlerts()) {
                foreach ($extensions as $name) {
                    ...
                    if (array_key_exists('status', $about) && in_array(EXTENSION_REQUIRES_UPDATE, $about['status'])) {
                        $this->Page->pageAlert(
                            tr('An extension requires updating.') . ' <a href="' . SYMPHONY_URL . '/system/extensions/">' . tr('View extensions') . '</a>'
                        );
                        break;
                    }
                }
            ...
            if (Symphony::isInstallerAvailable() && $this->canAccessAlerts()) {
                try {
                    ...
                    if (Symphony::isUpgradeAvailable()) {
                        ...
                        $message = tr('Your Symphony installation is up to date, but the installer was still detected. For security reasons, it should be removed.') . ' <a href="' . URL . '/install/?action=remove">' . tr('Remove installer?') . '</a>';
                    }
                } catch (Exception $e) {
                    // Can't detect update Symphony version
                    $message = tr('An update script has been found in your installation.') . ' <a href="' . URL . '/install/">' . tr('View update.') . '</a>';
                }
    ...
    public function getPageCallback($page = null)
    {
        ...
        if ($bits[0] == 'login') {
            ...
            $callback = array(
                'driver' => 'login',
                'context' => $context,
                'classname' => '\\SymphonyCms\\Pages\\Content\\LoginPage',
                'pageroot' => '/login/'
            );
