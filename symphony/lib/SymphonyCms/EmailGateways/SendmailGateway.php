...
class SendmailGateway extends EmailGateway
{
    ...
    public function send()
    {
        ...
        try {
            ...
            $from = empty($this->sender_name)
                    ...
                    : EmailHelper::qEncode($this->sender_name) . ' <' . $this->sender_email_address . '>';
            ...
            // Send the email
            mail($recipient_list, $subject, $this->_body, $header_fields, "-f{$this->sender_email_address}");
        } catch (\Exception $e) {
    ...
    public function getPreferencesPane()
    {
        ...
        $group->appendChild(new XMLElement('legend', tr('Email: Sendmail')));
        ...
        $label->appendChild(Widget::Input('settings[emailsendmail][from_name]', $this->sender_name));
        $div->appendChild($label);

        $label = Widget::Label(tr('From Email Address'));
        $label->setAttribute('class', 'column');
        $label->appendChild(Widget::Input('settings[emailsendmail][from_address]', $this->sender_email_address));
        $div->appendChild($label);
