...
namespace SymphonyCms\EmailGateways;
...
class SMTPGateway extends EmailGateway
{
    ...
    public function send()
    {
        ...
        try {
            ...
            $from = (empty($this->sender_name) ? $this->sender_email_address : EmailHelper::qEncode($this->sender_name) . ' <' . $this->sender_email_address . '>');
            ...
            $this->_SMTP->sendMail(is_null($this->_envelope_from)?$this->sender_email_address:$this->_envelope_from, $this->_recipients, $this->_subject, $this->_body);
    ...
    {
        if (preg_match('%[\r\n]%', $envelope_from)) {
            throw new EmailValidationException(tr('The Envelope From Address can not contain carriage return or newlines.'));
        }
    ...
    public function getPreferencesPane()
    {
        ...
        $group->appendChild(new XMLElement('legend', tr('Email: SMTP')));

        $div = new XMLElement('div');
        $div->setAttribute('class', 'two columns');

        $label = Widget::Label(tr('From Name'));
        $label->setAttribute('class', 'column');
        $label->appendChild(Widget::Input('settings[email_smtp][from_name]', $this->sender_name));
        $div->appendChild($label);

        $label = Widget::Label(tr('From Email Address'));
        $label->setAttribute('class', 'column');
        $label->appendChild(Widget::Input('settings[email_smtp][from_address]', $this->sender_email_address));
        $div->appendChild($label);
        ...
        $options = array(
            array('no',$this->_secure == 'no', tr('No encryption')),
            array('ssl',$this->_secure == 'ssl', tr('SSL encryption')),
            array('tls',$this->_secure == 'tls', tr('TLS encryption')),
        );
        $select = Widget::Select('settings[email_smtp][secure]', $options);
        $label->appendChild($select);
        $group->appendChild($label);

        $group->appendChild(new XMLElement('p', tr('For a secure connection, SSL and TLS are supported. Please check the manual of your email provider for more details.'), array('class' => 'help')));
        ...
        $label->setValue(tr('%s Requires authentication', array($input->generate())));
        $group->appendChild($label);

        $group->appendChild(new XMLElement('p', tr('Some SMTP connections require authentication. If that is the case, enter the username/password combination below.'), array('class' => 'help')));
