 ...
 */
class ExtensionManager implements FileResourceInterface
{
    /**
    ...
    /**
     * This method will populate the `$_subscriptions` variable from
     * the `tbl_extension` and `tbl_extensions_delegates` tables.
     */
    public function getSubscriptions()
    {
        if (empty(self::$_subscriptions)) {
            $subscriptions = Symphony::get('Database')->fetch(
    ...
    {
        return '\\SymphonyCms\\Extensions\\' . $name;
    }

    /**
     * Given a name, return the path to the driver of the Extension.
     *
     * @see toolkit.ExtensionManager#getClassPath()
     * @param string $name
     *  The extension folder
     * @return string
     */
    ...
    {
        return self::getClassPath($name) . '\\' . $name;
    }
     ...
     */
    private static function buildExtensionList($update = false)
    {
        if (empty(self::$_extensions) || $update) {
            self::$_extensions = Symphony::get('Database')->fetch("SELECT * FROM `tbl_extensions`", 'name');
        }
    }
    ...
    public static function fetchStatus($about)
    {
        ...
        if (isset($about['handle']) && self::requiresUpdate($about['handle'], $about['version'])) {
     ...
     * @return boolean
     */
    private static function requiresInstallation($name)
    {
        self::buildExtensionList();
     ...
     *  updating, false.
     */
    private static function requiresUpdate($name, $file_version)
    {
    ...
    public static function enable($name)
    {
        ...
        if (self::requiresInstallation($name) && $obj->install() === false) {
            // If the installation failed, run the uninstall method which
            // should rollback the install method. #1326
            $obj->uninstall();
            return false;
        } elseif (($about = self::about($name)) && ($previousVersion = self::requiresUpdate($name, $about['version'])) !== false) {
        ...
        if (is_null($id)) {
            Symphony::get('Database')->insert($fields, 'tbl_extensions');
            self::buildExtensionList(true);
        } else {
            // Extension is installed, so update!
            Symphony::get('Database')->update($fields, 'tbl_extensions', " `id` = '$id '");
        }
    ...
    public static function disable($name)
    {
        ...
        self::canUninstallOrDisable($obj);
    ...
    public static function uninstall($name)
    {
        ...
        self::removeDelegates($name);
        Symphony::get('Database')->delete('tbl_extensions', sprintf(" `name` = '%s' ", $name));
    ...
    public static function registerDelegates($name)
    {
        ...
        Symphony::get('Database')->delete('tbl_extensions_delegates', " `extension_id` = '$id ' ");
    ...
    public static function removeDelegates($name)
    {
        ...
        $delegates = Symphony::get('Database')->fetchCol(
            'id',
            sprintf(
        ...
        if (!empty($delegates)) {
            Symphony::get('Database')->delete('tbl_extensions_delegates', " `id` IN ('". implode("', '", $delegates). "') ");
        }
     ...
     * @return boolean
     */
    private static function canUninstallOrDisable(Extension $obj)
    {
        ...
        if (is_dir(EXTENSIONS . "/{$extension_handle}/fields")) {
            foreach (glob(EXTENSIONS . "/{$extension_handle}/fields/field.*.php") as $file) {
                ...
                if (FieldManager::isFieldUsed($type)) {
                    throw new Exception(
                        tr('The field ‘%s’, provided by the Extension ‘%s’, is currently in use.', array(basename($file), $about['name']))
                        . ' ' . tr("Please remove it from your sections prior to uninstalling or disabling.")
                    );
                }
            }
        }
        ...
        if (is_dir(EXTENSIONS . "/{$extension_handle}/data-sources")) {
            foreach (glob(EXTENSIONS . "/{$extension_handle}/data-sources/data.*.php") as $file) {
                ...
                if (PageManager::isDataSourceUsed($handle)) {
                    throw new Exception(
                        tr('The Data Source ‘%s’, provided by the Extension ‘%s’, is currently in use.', array(basename($file), $about['name']))
                        . ' ' . tr("Please remove it from your pages prior to uninstalling or disabling.")
                    );
                }
            }
        }
        ...
        if (is_dir(EXTENSIONS . "/{$extension_handle}/events")) {
            foreach (glob(EXTENSIONS . "/{$extension_handle}/events/event.*.php") as $file) {
                ...
                if (PageManager::isEventUsed($handle)) {
                    throw new Exception(
                        tr('The Event ‘%s’, provided by the Extension ‘%s’, is currently in use.', array(basename($file), $about['name']))
                        . ' ' . tr("Please remove it from your pages prior to uninstalling or disabling.")
                    );
                }
            }
        ...
        if (is_dir(EXTENSIONS . "/{$extension_handle}/text-formatters")) {
            foreach (glob(EXTENSIONS . "/{$extension_handle}/text-formatters/formatter.*.php") as $file) {
                ...
                if (FieldManager::isTextFormatterUsed($handle)) {
                    throw new Exception(
                        tr('The Text Formatter ‘%s’, provided by the Extension ‘%s’, is currently in use.', array(basename($file), $about['name']))
                        . ' ' . tr("Please remove it from your fields prior to uninstalling or disabling.")
                    );
                }
            }
        }
    ...
    public static function notifyMembers($delegate, $page, array $context = array())
    {
        ...
        foreach ($services as $s) {
            ...
            Symphony::get('Profiler')->sample($delegate . '|' . $s['name'], PROFILE_LAP, 'Delegate', $queries);
        }
    }
    ...
    public static function listInstalledHandles()
    {
        if (is_null(self::$_enabled_extensions)) {
            self::$_enabled_extensions = Symphony::get('Database')->fetchCol(
                'name',
    ...
    public static function about($name, $rawXML = false)
    {
        ...
        if (file_exists(self::getClassPath($name) . '/extension.meta.xml')) {
            try {
                $meta = new DOMDocument;
                $meta->load(self::getClassPath($name) . '/extension.meta.xml');
                $xpath = new DOMXPath($meta);
                $rootNamespace = $meta->lookupNamespaceUri($meta->namespaceURI);

                if (is_null($rootNamespace)) {
                    throw new Exception(tr('Missing default namespace definition.'));
                } else {
                    $xpath->registerNamespace('ext', $rootNamespace);
                }
            } catch (Exception $ex) {
                Symphony::get('Engine')->throwCustomError(
                    tr('The %1$s file for the %2$s extension is not valid XML: %3$s', array(
            ...
            if (self::getClassName($name) != self::getClassName($xpath->evaluate('string(@id)', $extension))) {
                return array();
            }
            ...
            if ($release = $xpath->query("//ext:release[@version='$latest_release_version']", $extension)->item(0)) {
                ...
                $current_symphony_version = Symphony::get('Configuration')->get('version', 'symphony');
        ...
        if (!isset(self::$_pool[$name])) {
            ...
            if (!is_file($path)) {
                Symphony::get('Engine')->throwCustomError(
                    ...
                    tr('Symphony Extension Missing Error'),
    ...
    public static function cleanupDatabase()
    {
        ...
        $rows = Symphony::get('Database')->fetch("SELECT `name` FROM `tbl_extensions`");
        ...
        if (is_array($rows) && !empty($rows)) {
            foreach ($rows as $r) {
                ...
                if (!@is_dir($path)) {
                    Symphony::get('Database')->delete("tbl_extensions_delegates", " `extension_id` = $existing_id ");
                    Symphony::get('Database')->delete('tbl_extensions', " `id` = '$existing_id' LIMIT 1");
 ...
 * @var integer
 */
defineSafe('EXTENSION_REQUIRES_UPDATE', 13);
 ...
 * @var integer
 */
defineSafe('EXTENSION_NOT_COMPATIBLE', 14);
