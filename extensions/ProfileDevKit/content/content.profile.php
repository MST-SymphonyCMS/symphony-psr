<?php
	...
	class Content_ProfileDevKit extends DevKit {
		...
		public function __construct(){
			...
			$this->_title = tr('Profile');
			$this->_query_string = parent::buildQueryString(array('symphony-page', 'profile'));
		...
		protected function buildJump(XMLElement $wrapper) {
			...
			$list->appendChild($this->buildJumpItem(
				tr('Full Page Render Statistics'),
		...
		public function buildContent(XMLElement $wrapper) {
			...
			if ($this->_view == 'render-statistics') {
				...
				$this->_records = array(
					array(tr('Total Database Queries'), $this->_dbstats['queries'], null, null, false),
					array(tr('Slow Queries (> 0.09s)'), (string)count($this->_dbstats['slow-queries']), null, null, false),
					array(tr('Total Time Spent on Queries'), $this->_dbstats['total-query-time']),
					array(tr('Time Triggering All Events'), $event_total),
					array(tr('Time Running All Data Sources'), $ds_total),
					array(tr('XML Generation'), $xml_generation[1]),
					array(tr('XSLT Transformation'), $xsl_transformation[1]),
					array(tr('Output Creation Time'), Symphony::Profiler()->retrieveTotalRunningTime()),
					array(tr('Total Memory Usage'), General::formatFilesize(Symphony::Profiler()->retrieveTotalMemoryUsage()), null, null, false),
				);
