<?php
	...
	RewriteRule ^image\/(.+\.(jpg|gif|jpeg|png|bmp))\$ extensions/jit_image_manipulation/lib/image.php?param={$token} [B,L,NC]" . PHP_EOL . PHP_EOL;
				...
				Administration::instance()->Page->pageAlert(tr('An error occurred while installing %s. %s', array(tr('JIT Image Manipulation'), $ex->getMessage())), Alert::ERROR);
				return false;
			}
		}
		...
		public function update($previousVersion = false) {
			if(version_compare($previousVersion, '1.17', '<')) {
				...
				try {
					...
					if(!file_put_contents(DOCROOT . '/.htaccess', $htaccess)) {
						Administration::instance()->Page->pageAlert(tr('An error occurred while updating %s. %s', array(tr('JIT Image Manipulation'), $ex->getMessage())), Alert::ERROR);
						return false;
					}
				}
			}
	...
	-------------------------------------------------------------------------*/
				...
				foreach($recipes as $position => $recipe) {
					if(empty($recipe['name'])) {
						$this->recipes_errors[$position] = array(
							'missing' => tr('This is a required field.')
						);
						break;
					}
					...
					if (!in_array($recipe['url-parameter'], $handles)) {
						...
						$this->recipes_errors[$position] = array(
							'invalid' => tr('A recipe with this regular expression already exists.')
						);
					}
					else {
						// handle does exist => set error
						$this->recipes_errors[$position] = array(
							'invalid' => tr('A recipe with this handle already exists. All handles must be unique.')
						);
					}
				}
			}
		...
		public function createRecipeDuplicatorTemplate($mode = '0', $position = '-1', $values = array(), $error = false){
			...
			$label = (!empty($values['name'])) ? $values['name'] : tr('New Recipe');
			...
			// Name
			$label = Widget::Label(tr('Name'), null, 'column');
			...
			// Handle
			$label_text = $mode === 'regex' ? tr('Regular Expression') : tr('Handle') . '<i>e.g. /image/{handle}/path/to/my-image.jpg</i>';
			$label = Widget::Label(tr($label_text), null, 'column');
			...
			// regex mode
			if ($mode === 'regex') {
				$label = Widget::Label(tr('JIT Parameter'));
			...
			$group->setAttribute('class', 'two columns');
			if ($mode !== '0') {
				$label = Widget::Label(tr('Image quality'), null, 'column');
				$label->appendChild(new XMLElement('i', tr('Optional')));
			...
			if ($mode !== 'regex') {
				...
				$label->setValue($input->generate() . ' ' . tr('External Image'));
	...
	-------------------------------------------------------------------------*/
		...
		public function appendPreferences($context){
			...
			if (!file_exists(WORKSPACE . '/jit-image-manipulation/')) {
				Administration::instance()->Page->pageAlert(tr('The JIT configuration folder, %s, does not exist. You will not be able to save recipes and trusted sites.', array('<code>/workspace/jit-image-manipulation/</code>')), Alert::ERROR);
			}
			...
			$group->appendChild(new XMLElement('legend', tr('JIT Image Manipulation')));
			$group->appendChild(
				new XMLElement('p', tr('Recipes are named rules for the JIT settings.'), array('class' => 'help'))
			);

			// recipes duplicator
			$group->appendChild(new XMLElement('p', tr('Recipes'), array('class' => 'label')));
			$div = new XMLElement('div', null, array('class' => 'frame'));
			$duplicator = new XMLElement('ol');
			$duplicator->setAttribute('class', 'jit-duplicator');
			$duplicator->setAttribute('data-add', tr('Add recipe'));
			$duplicator->setAttribute('data-remove', tr('Remove recipe'));
			...
			$label->setValue($input->generate() . ' ' . tr('Disable dynamic URLs and use named recipes only'));
			...
			$label->setValue($input->generate() . ' ' . tr('Disable upscaling of images beyond the original size'));
			$group->appendChild($label);

			// textarea for trusted sites
			$label = Widget::Label(tr('Trusted Sites'));
			$label->appendChild(Widget::Textarea('jit_image_manipulation[trusted_external_sites]', 5, 50, $this->trusted()));

			$group->appendChild($label);
			$group->appendChild(new XMLElement('p', tr('Leave empty to disable external linking. Single rule per line. Add * at end for wild card matching.'), array('class' => 'help')));
		...
		public function __SavePreferences($context){
			...
			if ($trusted_saved == self::__ERROR_TRUSTED__) {
				$context['errors']['jit-trusted-sites'] = tr('An error occurred while saving the JIT trusted sites. Make sure the trusted sites file, %s, exists and is writable.', array('<code>/workspace/jit-image-manipulation/trusted-sites</code>'));
			};
			...
			if ($recipes_saved == self::__ERROR_SAVING_RECIPES__) {
				$context['errors']['jit-recipes'] = tr('An error occurred while saving the JIT recipes. Make sure the recipes file, %s, exists and is writable.', array('<code>/workspace/jit-image-manipulation/recipes.php</code>'));
			};
			// there were duplicate recipes handles
			if ($recipes_saved == self::__INVALID_RECIPES__) {
				$context['errors']['jit-recipes'] = tr('An error occurred while saving the JIT recipes.');
			};
		}
	}
