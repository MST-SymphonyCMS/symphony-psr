	...
	Class extension_export_ensemble extends Extension{
		...
		public function install(){
			if(!class_exists('ZipArchive')){
				if(isset(Administration::instance()->Page)){
					Administration::instance()->Page->pageAlert(tr('Export Ensemble will not be able to download ZIP archives, since the "<a href="http://php.net/manual/en/book.zip.php">ZipArchive</a>" class is not available. To enable ZIP downloads, compile PHP with the <code>--enable-zip</code> flag.'), Alert::ERROR);
				}
			}
			return true;
		}
		...
		public function appendPreferences($context){
			...
			if(!is_dir(DOCROOT . '/install/')){
				...
				$no_install_dir_warning = ' <strong>Warning: It appears you do not have an <code>install</code> directory.</strong> ' . tr($no_install_dir_message);
			}

			// If Export Ensemble button is pressed
			// Test whether the install directory exists
			if(isset($_POST['action']) && !is_dir(DOCROOT . '/install/')){
				Administration::instance()->Page->pageAlert(tr($no_install_dir_message), Alert::ERROR);
			}
			else {
			...
			$group->appendChild(new XMLElement('legend', tr('Export Ensemble')));


			$div = new XMLElement('div', null, array('id' => 'file-actions', 'class' => 'label'));
			$span = new XMLElement('span', null, array('class' => 'frame'));

			$span->appendChild(new XMLElement('button', tr('Save Install Files'), array('name' => 'action[save-install-files]', 'type' => 'submit')));

			if(!class_exists('ZipArchive')){
				$no_zip_warning = ' <strong>' . tr('Warning: It appears you do not have the "ZipArchive" class available. To enable ZIP download, ensure that PHP is compiled with <code>--enable-zip</code>') . '</strong>';
			}
			else{
				$span->appendChild(new XMLElement('button', tr('Download ZIP'), array('name' => 'action[download-zip]', 'type' => 'submit')));
			}

			$div->appendChild($span);

			$div->appendChild(new XMLElement('p', tr('Save (overwrite) install files or package entire site as a <code>.zip</code> archive for download.' . $no_zip_warning . $no_install_dir_warning), array('class' => 'help')));
		...
		private function __saveInstallFiles(){
			...
			if(false !== @file_put_contents(DOCROOT . '/install/includes/install.sql', $sql_schema));
				...
				Administration::instance()->Page->pageAlert(tr('An error occurred while trying to write the <code>install.sql</code> file. Check the file permissions.'), Alert::ERROR);
				return;
			}
			if(false !== @file_put_contents(DOCROOT . '/install/includes/config_default.php', $config_template));
			else {
				Administration::instance()->Page->pageAlert(tr('An error occurred while trying to write the <code>config_default.php</code> file. Check the file permissions.'), Alert::ERROR);
				return;
			}
			if(false !== @file_put_contents(DOCROOT . '/workspace/install.sql', $sql_data));
			else {
				Administration::instance()->Page->pageAlert(tr('An error occurred while trying to write the <code>workspace/install.sql</code> file. Check the file permissions.'), Alert::ERROR);
				return;
			}

			Administration::instance()->Page->pageAlert(tr('The install files were successfully saved.'), Alert::SUCCESS);
		}
		...
		private function __createDefaultConfigFile(){
			...
			Symphony::Configuration()->remove('emailsendmail');
		...
		private function __createZipArchive($config_template, $sql_schema, $sql_data){
			if (!is_writable(DOCROOT . '/manifest/tmp')) {
				Administration::instance()->Page->pageAlert(tr('Check permissions for the /manifest/tmp directory.'), Alert::ERROR);
			}
			else {
