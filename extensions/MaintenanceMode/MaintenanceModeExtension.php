<?php
	...
	Class extension_maintenance_mode extends Extension {
		...
		public function appendPreferences($context) {
			...
			$group->appendChild(new XMLElement('legend', tr('Maintenance Mode')));
			...
			$label->setValue($input->generate() . ' ' . tr('Enable maintenance mode'));
			$group->appendChild($label);

			// Append help
			$group->appendChild(new XMLElement('p', tr('Maintenance mode will redirect all visitors, other than developers, to the specified maintenance page. To specify a maintenance page, give a page a type of <code>maintenance</code>'), array('class' => 'help')));
		...
		public function __toggleMaintenanceMode() {
			if($_REQUEST['action'] == 'toggle-maintenance-mode') {
				...
				// Redirect
				\redirect((isset($_REQUEST['\SymphonyCms\Utilities\redirect']) ? SYMPHONY_URL . $_REQUEST['\SymphonyCms\Utilities\redirect'] : Administration::instance()->getCurrentPageURL() . '/'));
			}
		}
		...
		public function __appendAlert($context) {
			...
			if(Symphony::Configuration()->get('enabled', 'maintenance_mode') == 'yes') {
				Administration::instance()->Page->pageAlert(
					tr('This site is currently in maintenance mode.') . ' <a href="' . SYMPHONY_URL . '/system/preferences/?action=toggle-maintenance-mode&amp;\SymphonyCms\Utilities\redirect=' . \SymphonyCms\Utilities\getCurrentPage() . '">' . tr('Restore?') . '</a>',
					Alert::NOTICE
				);
			}
		}
			...
			if(!Symphony::Engine()->isLoggedIn() && Symphony::Configuration()->get('enabled', 'maintenance_mode') == 'yes'){
				...
				if(empty($context['row'])) {
					Symphony::Engine()->customError(
						tr('Website Offline'),
						tr('This site is currently in maintenance. Please check back at a later date.')
					);
				}
			}
		}
