<?php
	...
	if (!is_dir(BITTER_CACHE_PATH) && General::realiseDirectory(BITTER_CACHE_PATH) === false){
		throw new Exception(tr('Failed to create cache folder. Please check "%s" is writable.', array(BITTER_CACHE_PATH)));
	}

	elseif(!is_writable(BITTER_CACHE_PATH)){
		throw new Exception(tr('Cache folder is not writable. Please check permissions on "%s".', array(BITTER_CACHE_PATH)));
	}
	...
	class Content_DebugDevKit_Debug extends DevKit {
		...
		public function __construct(){
			...
			$this->_title = tr('Debug');
			$this->_query_string = parent::buildQueryString(array('symphony-page', 'debug'));
		...
		protected function buildJump(XMLElement $wrapper) {
			...
			$utilities = $this->buildUtilityList($this->__findUtilitiesInFile($this->_pagedata['filelocation']), 1, $this->_view);
		...
		public function buildContent(XMLElement $wrapper) {
			...
			if ($this->_view == 'params') {
				$wrapper->appendChild($this->buildParams($this->_param));
		...
		protected function appendSource(XMLElement $wrapper, $source, $language = 'xml') {
			...
			if($this->_view == 'xml') {
				...
				$viewRaw->appendChild(new XMLElement('span',tr('Plain XML')));
		...
		protected function buildParams($params) {
			...
			$params = General::arrayMapRecursive(array('General', 'sanitize'), $params);
		...
		protected function buildUtilityList($utilities, $level=1, $view = null) {
			...
			foreach ($utilities as $u) {
				...
				if (is_array($child_utilities) && !empty($child_utilities)) {
					$item->appendChild($this->buildUtilityList($child_utilities, $level + 1, $view));
				}
